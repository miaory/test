...
#     history_dir = os.path.join(best_dir, "history")
...
#             logger.info(f"[VAL] ā={a_mean_cv:.1f}, b̄={b_mean_cv:.1f}")
...
#         a_mean_cv = (a_ + 128.0).mean().item()
#         b_mean_cv = (b_ + 128.0).mean().item()
#         logger.info(f"[INFER] ā={a_mean_cv:.1f}, b̄={b_mean_cv:.1f}")
#     mean_psnr = float(np.mean(psnr_list)) if psnr_list else 0.0
#     if psnr_list:
...
#     # 训练控制项
#     val_every    = max(1, _as_int(tr.get("val_every", 1), 1))
#     use_ema      = bool(tr.get("ema", False))
#     ema_decay    = _as_float(tr.get("ema_decay", 0.999), 0.999)
#     cosine_T0    = _as_int(tr.get("cosine_T0", 150), 150)
#     cosine_Tmult = _as_int(tr.get("cosine_Tmult", 2), 2)
...
#     gate_reg_lambda = _as_float(tr.get("gate_reg", 1e-4), 1e-4)
...
#     # 调度器：Cosine 重启
#     lr_sch = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
#         optimizer, T_0=cosine_T0, T_mult=cosine_Tmult, eta_min=1e-7
#     )

#     criterion_main = import_loss(opt.model_task).to(opt.device)

#     # 颜色一致性（窗口门控 + warm-up + ★scale）
#     lc = opt.config.get("loss", {}).get("color", {})
#     use_color = bool(lc.get("enable", True))
#     color_warm_epochs = _as_int(lc.get("warmup_epoch", 10), 10)
#     color_scale = _as_float(lc.get("scale", 1.0), 1.0)  # ★ 新增总系数
...
#                     loss = rec_loss + color_scale * cw * col_total   # ★ 使用 color_scale
#                 else:
#                     loss = rec_loss

#                 # 门控“恒等”正则
#                 if gate_reg_lambda > 0 and epoch <= gate_reg_epochs:
#                     loss = loss + gate_reg_lambda * gate_identity_reg(model)
...
#             logger.info(f"[E{epoch}] train_loss={np.mean(running):.4f}, val_psnr={val_psnr:.3f}")
#         elif val_loader is None:
#             logger.info(f"[E{epoch}] train_loss={np.mean(running):.4f}, no val_loader")

#         # ===== 保存最好（best + history + slim + JSONL） =====
#         if val_psnr != float('-inf') and val_psnr > best_psnr:
#             best_psnr = val_psnr

#             # 选择要保存的 state_dict（EMA 优先）
#             if ema is not None:
#                 ema.apply_to(model)
