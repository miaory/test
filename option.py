...
def get_option():
    ...
    parser.add_argument(
        ...
        default='isp',
        type=str,
        choices=['isp', 'lle', 'sr', 'lle_psnr'],  # ★ 添加 lle_psnr
    ...
    parser.add_argument(
        '--ckpt',  # Add argument for checkpoint path
        type=str,
        default='',
        help='Path to the checkpoint to resume training or for testing.'
    )
...
def opt_format(opt):
    ...
    if opt.task == 'train':
        ...
        # demo 或 test 自动使用训练好的模型路径
        # default_model_dir = os.path.join(opt.root, 'experiments/2025-11-09 16-07-19 train_lle/models')
        # opt.save_model_dir = default_model_dir
        opt.save_model_dir = os.path.dirname(opt.config['test']['ckpt'])
        if not os.path.exists(opt.save_model_dir):
            raise FileNotFoundError(f'Default model path not found: {opt.save_model_dir}')

    # If checkpoint is provided, set it in the config
    if opt.ckpt:
        opt.config['train']['ckpt'] = opt.ckpt
