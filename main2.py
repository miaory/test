...
try:
    ...
    lpips = None
    print("[WARN] 未找到 lpips 库，将跳过 LPIPS 计算。请使用 'pip install lpips' 安装。")
...
def _get_scheduler(optimizer, tr_cfg, logger):
    ...
    if sch == "plateau":
        ...
        return sch, torch.optim.lr_scheduler.ReduceLROnPlateau(
...
def validate(model, val_loader, device, logger, color_loss=None, max_eval: int = 0):
    ...
    with torch.no_grad():
        ...
        for batch in _tqdm(val_loader, desc="[VAL]"):
            ...
            # ---------- PSNR 计算：与代码一保持一致 ----------
            # mse over H, W -> shape [B, C]
            mse = ((pred - gt) ** 2).mean(dim=(2, 3))
            # 10 * log10(1 / mse)，再对 batch 和通道平均
            psnr = (1.0 / mse).log10().mean() * 10.0
...
def infer_and_maybe_save(model, data_loader, device, logger, save_images=False, save_dir=None, color_loss=None, prefix=""):
    ...
    for batch in _tqdm(data_loader, desc="[INFER]"):
        ...
        if gt is not None:
            # 这里如果你也想统一成“代码一风格 PSNR”，可以同样改成 mse→psnr
            mse = ((pred - gt) ** 2).mean(dim=(2, 3))
            psnr = (1.0 / mse).log10().mean() * 10.0
            psnr_list.append(psnr.item())
    if color_loss is not None and last_pred is not None:
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loader is not None and do_validate:
            ...
            if use_ema_flag and ema is not None:
                ema.apply_to(model)
                val_psnr = validate(
                    model, val_loader, opt.device, logger,
            ...
            logger.info(
                f"[E{epoch}] lr={_get_lr(optimizer):.6g}, "
...
# -------------------- 测试 (PSNR 改成与代码一一致 + LPIPS + Runtime + Score) --------------------
def test():
    opt = get_option()
    logger = Logger(opt)
    _maybe_set_cudnn(opt, logger)
    ...
    if lpips is not None:
        # 使用 VGG 作为主干网络 (标准设置)
        # verbose=False 防止打印模型加载信息
        loss_fn_lpips = lpips.LPIPS(net='vgg', verbose=False).to(opt.device)
        loss_fn_lpips.eval()
        logger.info("[TEST] LPIPS model initialized.")
    ...
    if opt.device.startswith("cuda"):
        logger.info("[TEST] Warming up GPU for accurate timing...")
        dummy = torch.randn(1, 3, 256, 256).to(opt.device)
        with torch.no_grad():
            for _ in range(10):
                _ = model(dummy)
    ...
    logger.info("[TEST] Starting inference loop...")
    with torch.no_grad():
        for batch in _tqdm(test_loader, desc="[TEST]"):
            ...
            if gt is not None:
                ...
                if loss_fn_lpips is not None:
                    ...
                    gt_norm = gt * 2.0 - 1.0
                    lp_val = loss_fn_lpips(pred_norm, gt_norm).item()
    ...
    # ★★★ 计算 Score (C = 26906) ★★★
    # Score = 2^PSNR / (C * Runtime)
    c_val = 26906
    if mean_time < 1e-6:
        mean_time = 0.1  # 防止除零
    score = (2 ** mean_psnr) / (c_val * mean_time)

    print("\n" + "=" * 40)
    print(f"  TEST RESULTS ({opt.model_task.upper()})")
    print("=" * 40)
    print(f"  PSNR    : {mean_psnr:.4f} dB")
    print(f"  LPIPS   : {mean_lpips:.4f}")
    print(f"  Runtime : {mean_time:.4f} ms (avg per batch)")
    print(f"  Score   : {score:.4f} (C={c_val})")
    if save_flag:
        print(f"  Output  : {out_dir}")
    print("=" * 40 + "\n")

    logger.info(f"[TEST] PSNR: {mean_psnr:.4f}, LPIPS: {mean_lpips:.4f}, Runtime: {mean_time:.4f}ms, Score: {score:.4f}")
